...
from graph_utils import reveert_map_node_embedding, load_hetero_nx_graph
from model_hetero import MANDOGraphClassifier as GraphClassifier
...
def node2vec(compressed_graph, dataset, feature_extractor, bugtype, device, repeat):
    ...
    model.eval()
    with torch.no_grad():
        output_feature = f'{ROOT}/logs/{TASK}/byte_code/{DATASET}/{BYTECODE}/{STRUCTURE}/{COMPRESSED_GRAPH}/node2vec/{bugtype}/cfg_pkl'
        logits, hiddens = model(X_val)
        for name,feature in zip(X_val,logits):
            write_pkl(name,feature,output_feature)
        logits, hiddens = model(X_train)
        for name,feature in zip(X_train,logits):
            write_pkl(name,feature,output_feature)
        logits = logits.to(device)
        # test_acc, test_micro_f1, test_macro_f1 = score(y_val, logits)
        test_results = get_classification_report(y_val, logits, output_dict=True)
...
def lstm(compressed_graph, dataset, bugtype, device, repeat):
    logs = f'{ROOT}/logs/{TASK}/source_code/lstm/{bugtype}/'
    if not os.path.exists(logs):
        os.makedirs(logs, exist_ok=True)
    output_models = f'{ROOT}/models/{TASK}/source_code/lstm/{bugtype}/'
    if not os.path.exists(output_models):
...
def main(device):
    for bugtype in bug_list:
        ...
        for i in range(REPEAT):
            ...
            compressed_graph = f'{ROOT}/ge-sc-data/source_code/{bugtype}/clean_{file_counter[bugtype]}_buggy_curated_0/cg_compressed_graphs.gpickle'
...
def write_pkl(name,feature,output):
    data = {
        'name': name,
        'feature': feature  # 创建一个3x3的随机矩阵
    }
