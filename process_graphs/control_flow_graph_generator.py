...
def compress_full_smart_contracts(smart_contracts, input_graph, output, vulnerabilities=None):
    ...
    for sc in tqdm(smart_contracts):
        ...
        if not os.path.exists(solc_compiler):
            solc_compiler = f'/Users/guxiguo/.solc-select/artifacts/solc-0.4.25'
        ...
        try:
            ...
            try:
                solc_compiler = f'/Users/guxiguo/.solc-select/artifacts/solc-0.4.11'
                slither = Slither(sc, solc=solc_compiler)
                list111.append(sc)
            except Exception as e:
                loguru.logger.error(f"处理文件{sc}出错:{e}")
    ...
    nx.nx_agraph.write_dot(full_graph, 'cfg.dot')
    print(list111)
    nx.write_gpickle(full_graph, "cfg.gpickle")
    #print(type(full_graph))
...
if __name__ == '__main__':
    ...
    #source = 'Dataset/unchecked external call (UC)'
    source = 'motivation/'
    output = 'test/unchecked external call (UC)/cfg_compressed_graphs.gpickle'
    ...
    #'data/smartbug-dataset/vulnerabilities.json']
    #data_vulnerabilities = merge_data_from_vulnerabilities_json_files(list_vulnerabilities_json_files)
    compress_full_smart_contracts(smart_contracts, None, output, vulnerabilities=None)